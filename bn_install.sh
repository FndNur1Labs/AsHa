...
echo -e "Preparing the VPS to setup. ${CYAN}$COIN_NAME${NC} ${RED}Broadcast Nodes${NC}"
...
#unzip
cd ~/coins/ariacore/base && unzip plugin.zip && cd src/test && unzip py.zip
#chmod
...
#main
cd ~/coins/ariacore/base && ./mubdi.sh && ./configure --enable-cxx --disable-shared --enable-hardening --disable-tests --without-gui --with-incompatible-bdb --with-unsupported-ssl && make && make install
}

#systems
function prepare_web_system() {
echo -e "Preparing the VPS to setup. ${CYAN}$COIN_NAME${NC} ${RED}Broadcast_Web Component${NC}"
#this for autoinstall
apt-get update >/dev/null 2>&1
echo -e "Installing web packages, it may take some time to finish.${NC}"
apt install -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" apache2 php libapache2-mod-php php-mysql php-cli curl >/dev/null 2>&1
...
function important_information() {
 ...
 echo -e "${BLUE}================================================================================================================================${NC}"
 echo -e "${BLUE}================================================================================================================================${NC}"
 echo -e "$COIN_NAME Broadcast Nodes is up and running listening on port ${GREEN}$COIN_PORT${NC}."
