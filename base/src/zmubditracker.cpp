...
CzMuBdITracker::CzMuBdITracker(std::string strWalletFile)
{
...
bool CzMuBdITracker::Archive(CMintMeta& meta)
{
...
bool CzMuBdITracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMuBdITracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMuBdITracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMuBdITracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzMuBdITracker::GetSerialHashes()
{
...
CAmount CzMuBdITracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMuBdITracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMuBdITracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMuBdITracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMuBdITracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMuBdITracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMuBdITracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMuBdITracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMuBdITracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMuBdITracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMuBdITracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMuBdITracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMuBdITracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMuBdITracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMuBdITracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMuBdITracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMuBdITracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzaria from DB\n", __func__, listDeterministicDB.size());
    }
...
void CzMuBdITracker::Clear()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

CzMuBdITracker::~CzMuBdITracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}
