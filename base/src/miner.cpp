            ...
            if (tx.IsZerocoinSpend()) {
                ...
                for (const CTxIn txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
						...
						//bool fUseV1Params = libzerocoin::ExtractVersionFromSerial(spend.getCoinSerialNumber()) < libzerocoin::PrivateCoin::PUBKEY_VERSION; --> Dark Protocol
                        //if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) --> Dark Protocol
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake())
            pwalletMain->zMuBdITracker->RemovePending(pblock->vtx[1].GetHash());
        ...
        if (fProofOfStake) {
            ...
            if (pblock->IsZerocoinStake()) {
                ...
                if (!pwallet->GetZerocoinKey(bnSerial, key)) {
                    LogPrintf("%s: failed to find zAR with serial %s, unable to sign block\n", __func__, bnSerial.GetHex());
                    continue;
                }

            //Sign block with the zMuBdI key
                if (!SignBlockWithKey(*pblock, key)) {
                    LogPrintf("MuBdIMiner(): Signing new block with zAR key failed \n");
                    continue;
                }
