...
CzMuBdIWallet* zwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 45088, 43000));
	...
	strUsage += HelpMessageOpt("-arstake=<n>", strprintf(_("Enable or disable staking functionality for AR inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zarstake=<n>", strprintf(_("Enable or disable staking functionality for zAR inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:45088"));
    ...
    strUsage += HelpMessageOpt("-zmubdibackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zAR backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 45133, 43100));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2010-%i Nur1Labs.Ltd"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
                                ...
                                if (tx.ContainsZerocoins()) {
                                    ...
                                    if (tx.IsZerocoinMint()) {
                                        for (auto& out : tx.vout) {
                                            ...
                                            //libzerocoin::PublicCoin coin(Params().Zerocoin_Params(pindex->nHeight < Params().Zerocoin_Block_V2_Start())); -> Dark Protocol
...
// ********************************************************* Step 8: load wallet
	...
	zwalletMain = new CzMuBdIWallet(pwalletMain->strWalletFile);
