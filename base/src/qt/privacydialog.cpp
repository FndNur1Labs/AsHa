...
void PrivacyDialog::on_pushButtonMintzMuBdI_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAriA is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zAriA...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zAriA in ") +
...
void PrivacyDialog::on_pushButtonSpendzMuBdI_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAriA is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
...
void PrivacyDialog::sendzMuBdI()
{
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zAriA</b>";
    ...
    if (!ZMuBdIControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZMuBdIControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
			if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zAriA require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zAriA"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
		if (receipt.GetStatus() == ZMUBDI_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zAriA require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zAriA"));
            ui->TEMintStatus->repaint();
            return;
    ...
    ZMuBdIControlDialog::setSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zAriA Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " AriA, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zAriA Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zMuBdITracker->GetMints(false);
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zAriA </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zAriA </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        ui->pushButtonMintzMuBdI->setToolTip(tr("zAriA is currently disabled due to maintenance."));

        // Spend zMuBdI
        ui->pushButtonSpendzMuBdI->setEnabled(false);
        ui->pushButtonSpendzMuBdI->setToolTip(tr("zAriA is currently disabled due to maintenance."));
    }
    else {
        // Mint zMuBdI
        ui->pushButtonMintzMuBdI->setEnabled(true);
        ui->pushButtonMintzMuBdI->setToolTip(tr("PrivacyDialog", "Enter an amount of AriA to convert to zAriA", 0));

        // Spend zMuBdI
        ui->pushButtonSpendzMuBdI->setEnabled(true);
        ui->pushButtonSpendzMuBdI->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}