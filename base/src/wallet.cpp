...
{
    return zMuBdITracker->HasSerial(bnSerial);
}
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate)
{
    ...
    bool fCheckZMuBdI = GetBoolArg("-zapwallettxes", false);
    if (fCheckZMuBdI)
        zMuBdITracker->Init();
        ...
        while (pindex) {
            ...
            BOOST_FOREACH (CTransaction& tx, block.vtx) {
                if (AddToWalletIfInvolvingMe(tx, &block, fUpdate))
	                ...
	                //If this is a zapwallettx, need to readd zar
            if (fCheckZMuBdI && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    ...
    CAmount nBalance = 0;
        vector<CMintMeta> vMints = zMuBdITracker->GetMints(true);
    ...
    return zMuBdITracker->GetBalance(false, false);
}
...
{
    return zMuBdITracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        set<CMintMeta> setMints = zMuBdITracker->ListMints(true, true, true);
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount)
{
    ...
    //zMuBdI
    if (GetBoolArg("-zarstake", true) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Add zMuBdI
        set<CMintMeta> setMints = zMuBdITracker->ListMints(true, true, true);
        ...
        for (auto meta : setMints) {
            ...
            if (meta.nHeight < chainActive.Height() - Params().Zerocoin_RequiredStakeDepth()) {
                std::unique_ptr<CZMuBdIStake> input(new CZMuBdIStake(meta.denom, meta.hashStake));
                listInputs.emplace_back(std::move(input));
            }
        }
    }
...
bool CWallet::MintableCoins()
{
	...
	CAmount nZMuBdIBalance = GetZerocoinBalance(false);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 MuBdI collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    vector<pair<CScript, CAmount> > vecSend;
    vecSend.push_back(make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 MuBdI collateral
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
	...
	// Make sure the wallet is unlocked and shutdown hasn't been requested
	    ...
	    CAmount nMinFee = 0;
            ...
            //Mark mints as spent
            if (stakeInput->IsZMuBdI()) {
                CZMuBdIStake* z = (CZMuBdIStake*)stakeInput.get();
    ...
    if (!txNew.vin[0].scriptSig.IsZerocoinSpend()) {
        ...
        for (const CTxOut& out : txNew.vout) {
            ...
            //libzerocoin::PublicCoin pubcoin(Params().Zerocoin_Params(false)); -> Dark Protocol
...
bool CWallet::CreateZMuBdIOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZMuBdI(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZMuBdIOutPut(denomination, outMint, dMint)) {
...
bool CWallet::MintToTxIn(CZerocoinMint zerocoinSelected, int nSecurityLevel, const uint256& hashTxOut, CTxIn& newTxIn, CZerocoinSpendReceipt& receipt, CBlockIndex* pindexCheckpoint)
{
    ...
    try {
        ...
        if (IsSerialKnown(spend.getCoinSerialNumber())) {
            ...
            if (!zMuBdITracker->HasSerialHash(hashSerial))
                return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

            CMintMeta meta = zMuBdITracker->Get(hashSerial);
            meta.isUsed = true;
            if (!zMuBdITracker->UpdateState(meta))
...
bool CWallet::CreateZerocoinSpendTransaction(CAmount nValue, int nSecurityLevel, CWalletTx& wtxNew, CReserveKey& reserveKey, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vSelectedMints, vector<CDeterministicMint>& vNewMints, bool fMintChange,  bool fMinimizeChange, CMuBdIAddress* address)
{
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zAriA"), nStatus);
        return false;
    }
    ...
    if (vSelectedMints.empty()) {
        setMints = zMuBdITracker->ListMints(true, true, true); // need to find mints to spend
        if(setMints.empty()) {
	...
	for (const auto& mint : vSelectedMints) {
        if (mint.GetVersion() < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            if (nSecurityLevel < 100) {
				nStatus = ZMUBDI_SPEND_V1_SEC_LEVEL;
                receipt.SetStatus(_("Version 1 zAriA require a security level of 100 to successfully spend."), nStatus);
                return false;
            }
        }
    }
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zMuBdITracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zMuBdITracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zMuBdITracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zMuBdITracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

string CWallet::GetUniqueWalletBackupName(bool fzMuBdIAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzMuBdIAuto ? "-autozariabackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}
...
void CWallet::ZMuBdIBackupWallet()
{
    ...
    if(!GetArg("-zariabackuppath", "").empty()) {
        filesystem::path customPath(GetArg("-zariabackuppath", ""));
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CMuBdIAddress* addressTo)
{
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zMuBdITracker->SetPubcoinNotUsed(hashPubcoin);
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zMuBdITracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zMuBdITracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
...
{
    CMintMeta meta;
    if (!zMuBdITracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zMuBdITracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zMuBdITracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zMuBdITracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zMuBdITracker->UpdateState(meta);
    } else {
