    ...
    for (CMintMeta meta : vMintsToFind) {
        ...
        for (auto& out : tx.vout) {
            if (!out.IsZerocoinMint())
                continue;
            //PublicCoin pubcoin(Params().Zerocoin_Params(meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION)); --> Dark Protocol
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if(tx.IsZerocoinSpend()){
			...
			// Check that zMuBdI mints are not already known
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
	    ...
	    set<CBigNum> setSerials;
            ...
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    ...
                    //PublicCoin coin(Params().Zerocoin_Params(false)); -> Dark Protocol
					PublicCoin coin(Params().Zerocoin_Params());
                    if (!TxOutToPublicCoin(out, coin, state))
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
     ...
     if (CZMuBdIStake* zMuBdI = dynamic_cast<CZMuBdIStake*>(stake)) {
        CBlockIndex* pindexFrom = zMuBdI->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zAR stake checksum", __func__);
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (stake->IsZMuBdI() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
...
int ActiveProtocol()
{
    // SPORK_14 is used for 25914 (v3.1.0+)
    if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
            return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;
    // SPORK_15 was used for 25913 (v3.0.5+), commented out now.
