...
CZMuBdIStake::CZMuBdIStake(const libzerocoin::CoinSpend& spend)
{
...
{
    //The unique identifier for a zMuBdI is a hash of the serial
...
bool CZMuBdIStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    ...
    //if (!pwallet->MintToTxIn(mint, nSecurityLevel, hashTxOut, txIn, receipt, libzerocoin::SpendType::STAKE, GetIndexFrom())) <- Dark Protocol
...
bool CZMuBdIStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zMuBdI that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZMuBdIOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zAriA output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zAriA", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZMuBdIOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zAriA output", __func__);
...
bool CZMuBdIStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
	CzMuBdITracker* zMuBdITracker = pwallet->zMuBdITracker.get();
    CMintMeta meta;
    if (!zMuBdITracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);
	
    zMuBdITracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
bool CMuBdIStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
