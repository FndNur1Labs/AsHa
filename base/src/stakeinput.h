...
public:
    explicit CZMuBdIStake(libzerocoin::CoinDenomination denom, const uint256& hashSerial)
    {
        this->denom = denom;
    ...
    explicit CZMuBdIStake(const libzerocoin::CoinSpend& spend);
    ...
    bool IsZMuBdI() override { return true; }
...
public:
    ...
    bool CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut = 0) override;
    bool CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal) override;
    bool IsZMuBdI() override { return false; }
};
