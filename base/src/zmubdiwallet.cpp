...
CzMuBdIWallet::CzMuBdIWallet(std::string strWalletFile)
{
	...
	//Check for old db version of storing zmubdi seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMuBdISeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
	    LogPrintf("%s: first run of zAriA wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMuBdISeed_deprecated()) {
                    LogPrintf("%s: Updated zAriA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zaria seed\n", __func__);
                }
            }
        }
    }
...
bool CzMuBdIWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    CWalletDB walletdb(strWalletFile);
    if (pwalletMain->IsLocked())
        return false;
    ...
    if (fResetCount)
        walletdb.WriteZMuBdICount(nCountLastUsed);
    else if (!walletdb.ReadZMuBdICount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMuBdIWallet::Lock()
{
    seedMaster = 0;
}

void CzMuBdIWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMuBdIWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
	...
	CBigNum bnValue;
        ...
        CKey key;
        SeedToZMuBdI(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMuBdIWallet::LoadMintPoolFromDB()
{
...
void CzMuBdIWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMuBdIWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMuBdIWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
	    ...
	    if (pwalletMain->zMuBdITracker->HasPubcoinHash(pMint.first)) {
...
bool CzMuBdIWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
        SeedToZMuBdI(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
	...
	// Add to zMuBdITracker which also adds to database
    pwalletMain->zMuBdITracker->Add(dMint, true);
...
void CzMuBdIWallet::SeedToZMuBdI(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMuBdIWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMuBdIWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZMuBdICount(nCountLastUsed);
...
void CzMuBdIWallet::GenerateDeterministicZMuBdI(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMuBdIWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMuBdIWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
