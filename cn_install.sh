...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='asha.conf'
CONFIGFOLDER='/root/.asha'
COIN_DAEMON='ashad'
COIN_CLI='asha-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ='http://nur1labs.net/lf/asha.zip'
COIN_ZIP=$(echo $COIN_TGZ | awk -F'/' '{print $NF}')
COIN_NAME='asha'
COIN_EXPLORER='http://chain.asha.org'
COIN_PORT=45032
RPC_PORT=45132
...
#unzip
cd ~/coins/ashacore/base && unzip plugin.zip && cd src/test && unzip py.zip
#chmod
cd ~/coins/ashacore/base && chmod 755 -R build-aux && chmod 755 -R depends && chmod 755 -R qa && chmod 755 -R share && chmod 755 -R mubdi.sh
cd ~/coins/ashacore/base && chmod 755 -R src/leveldb && chmod 755 -R src/univalue
#main
cd ~/coins/ashacore/base && ./mubdi.sh && ./configure --enable-cxx --disable-shared --enable-hardening --disable-tests --without-gui --with-incompatible-bdb --with-unsupported-ssl && make && make install
}
